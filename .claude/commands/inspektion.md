...
`server.ts` alone is ~8600 lines; a pulse that sweeps everything reads nothing. Read your register (below) and take the revier that has gone longest untouched, unless something in the last Rundgang or a red audit obviously points elsewhere — then say in one line why you jumped the rotation.
...
1. **Ledger-Anomalie** — a *ratio* or *silence* in the trails that smells wrong: `audit.jsonl`, `lane-outcomes.jsonl`, `post-land-audits.jsonl`, `steward-journal.jsonl`. Your access path: the digest carries `ledgers`, `slotHealth` and `continuity` — the raw routes (`/api/audit`, `/api/slot-stats`) are owner-only and answer your token 403. Where the digest's whitelist cut is not enough, the trail files are readable read-only at the main checkout (never write outside this worktree). Method: count, group, compare against what the code says *should* happen — then open that code. Example shape: an event that fires 196× with one value and 1× with the other, where the code's comment claims the rare one is the normal path. Start from the two derived views rather than re-aggregating by hand: `slotHealth` answers whether slots keep their identity, fall over, and how their sessions end; `continuity` answers whether they get attended. Both report what they deliberately excluded — read those counters, an exclusion spike is itself a finding.
...
5. **Grenze ohne Netz** — a boundary that takes input and does not validate it, or a path with no e2e check. Start from what the suites *do* cover (`e2e/*.ts` under the `fleet-e2e.ts` runner, plus the four single-file harnesses), and look at what sits next to it uncovered. Say plainly which suite you read.
...
Your register lives in the steward journal on the SERVER, not as a file in this worktree — the worktree is disposable by design (it was replaced on 2026-08-05 and the old file-register silently stayed behind in the dead tree; the legacy rows sit read-only in the main checkout's `inspektion-register.jsonl` until the owner migrates them). **Read it first**, always: `GET /api/steward/journal?kind=inspektion&tail=50` with your token. A finding already in it, whatever its verdict, is not filed again — a `dismissed` finding that returns every pulse is this channel's worst failure mode, worse than missing one. The 50-row tail is a recency window, not your memory: before filing any finding whose `key` you have derived, ask the log for exactly that key — `GET /api/steward/journal?kind=inspektion&ref=<key>&tail=5` — a hit, whatever its verdict, means it is registered even if it left the window long ago. Append after this pulse, one line per finding you *considered seriously*, including the ones you refuted — `POST /api/steward/journal` with your token and:

```
{"kind":"inspektion","revier":<1-5>,"key":"<stable-slug>","titel":"<one line>","cite":"<file:line>","verdict":"filed|refuted|observation|kandidat","task":"<id or null>"}
```

`key` must be stable across pulses — derive it from the thing, not from your phrasing (`self-heal-resume-ratio`, not `weird-numbers-in-audit`; shape `[a-z0-9][a-z0-9._-]{0,63}`, server-enforced). That slug is the whole dedup mechanism; a sloppy one silently re-files.

The server stamps `ts` itself — the first live pulse invented round-minute times that predate its own `auto_fire` by an hour, and a register whose timestamps are fiction cannot be audited against the trail; a server stamp closes that class. Register writes are capped per hour (an honest inspection's volume fits comfortably; a loop does not) — a 429 is a stop-and-surface signal, never something to wait out inside the pulse.

Then file **at most 1–2** — the ones the owner would be most annoyed to lose — via `POST /api/steward/tasks` with your token, `{"text": "<self-contained>", "ref": "<the same slug as the register key>"}` (defaults to `kind:"note"` — never dispatchable, correct for findings; claim `{"kind":"lane"}` only for a genuinely runnable work brief). The shared slug means the queue dedups against your own register vocabulary: a repeat filing of a live condition answers `{"dedup":true}` — treat that as "already surfaced", never as an error. He reads the text in the queue with none of this pulse's context: name the file:line, what is wrong or missing, the cost, and what decision is his. Include your confidence honestly — a `kandidat` that reads like a defect is worse than not filing. You file `pending` only; you never queue and never act. The server caps open steward proposals at 10; hitting the cap is itself something to surface, never to force past.
