...
- Three ledger signatures belong in section 1 and nowhere else: a **red audit** whose land has not since been resolved; a **land older than ~30 min with no audit row at all** while `auditConfigured` is true (tier 2's queue lives in the server's memory and dies with it, so that silence is indistinguishable from "nothing landed" — nobody but you is looking); a **shadow `would_stop`**. None of the three is an action for you — each is a candidate for his decision. Quiet trails say nothing, and then so do you.
