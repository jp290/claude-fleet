...
Two hard bounds, because this channel has **no de-duplication yet**: (1) a condition that persists (a lane awaiting-human for days) will re-file every pulse — so this pulse is run **manually and watched**, NEVER scheduled, until server-side `ref`-dedup lands; unwatched + no-dedup is exactly how the review buffer müllt sich zu. (2) The server caps open steward proposals at 10; hitting the cap is itself a signal to surface, never something to force past. Re-filing a duplicate you recognize from a previous pulse is knowingly accepted for now — but prefer to let the persistent condition ride in section-1 *display* rather than re-filing it as a fresh task each time.
