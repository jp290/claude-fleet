...
optional `card{ziel, surface{files, symbols}, done, verify, verboten, size, after?}` (`ROLLE` travels as the
top-level spawn triple; `size` is `klein`/`mittel`/`gross`, the row's weight against the land-wave
budget — absent weighs `mittel`; `after` lists the queue row ids the text orders this row after, the
same ids a `NACH:` header names). It is validated by `card-extract.ts#validateCard` against the row's repo;
...
bunx tsc --noEmit --strict --target esnext --module esnext --moduleResolution bundler --types bun \
  e2e/pins.ts src/client.ts src/share.ts src/helper.ts src/hub.ts server.ts fleet-e2e.ts \
  fleet-e2e-claude-gate.ts fleet-e2e-clean-review.ts fleet-e2e-security.ts fleet-e2e-postland-audit.ts \
  fleet-e2e-harness.ts merge-prompt.ts acceptance-probe.ts graph-coverage.ts land-collision-stats.ts \
  land-quality.ts lane-context-cost.ts drills/drill-3-clean-review.ts
bun run build
./e2e-clean-review.sh
./e2e-security.sh
./e2e-claude-gate.sh
