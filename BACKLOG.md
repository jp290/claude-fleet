...
| ~~P-9~~ | **SHIPPED (2026-08-04)** — `tickDispatch` compiles the queued text through the enhancer (`runEnhance` + `briefPayload` of the fresh lane, raw-text fallback) before injecting; `laneOwnerPrompts` now derives `briefHash` from the first owner/auto prompt, so a dispatched lane's provenance stops rounding to `null` (`ownerPrompts` stays owner-only — attention proxy). Proven in `e2e/tasks.ts` (compiled-brief check against the fakeenh stand-in) | — | done | knowledge-layers §4 |
