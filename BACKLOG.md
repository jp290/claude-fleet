...
**2026-07-25 — steward nudge designed (owner's idea), + the corpus re-analysis it needs. Doc: `docs/steward-nudge.md`. NOT built.** The steward fires a short, **content-free**, purely positive trigger ("gib dir Mühe") at a lane on a surface signal, and the *session* supplies the correction from its own brief + state. This splits the sharpen function at the seam that matters — steward owns **timing only**, never content — so it does not become the `situation-type → mechanism` lookup that THE GUARD (`~/.claude/knowledge/sharpen-corpus/model.md`) names as the specific error the corpus exists to prevent. It widens playbook #3 `continue-nudge` (which already has the required manually-proven predecessor); it is not a new intervention type. **"Purely positive" is a safety property, not a tone**: only a content-free trigger keeps the corpus's boundary condition ("no gap → sharpen idles, harmless when nothing changed"), because a message carrying a diagnosis will be conformed to even when the diagnosis is wrong. **Known limit:** the trigger can only ever mean *more* — `ASK→ACT`, `scope-creep→discipline` and the stop cases stay with `stuck-looping`/`awaiting-human` (escalate, never improvise). **Gate on measurement first:** re-run `rebuild.py` over the transcripts and, per corpus record, reconstruct whether a *surface* signal existed at that moment → recall, precision, and the more/stop split. Two things are newly possible that were not in the 2026-07-23 pass: lane **outcomes** now exist (the ledger had no outcome column before), and the 2026-07-24/25 session adds ~10 richly documented instances. Carry the corpus caveats forward (LLM-tagged claims, 7/182 stitched quotes); `situations.jsonl` stays ground truth. Measurement is free: steward sends are their own prompt-journal source (currently **0**), and `ownerPrompts` in the outcome row already counts human corrections (both lanes so far: 2).

**2026-07-24 (evening) — ③ `🔍 review` agent shipped (`ab59a71` + `7e5c777`), and the ledger recorded its FIRST row — which proved the recorder defective.** Item 14 Phase 2's review agent is live: owner-only, click-only, advisory `POST/GET /api/slots/:id/review` over the slot's own diff, mirroring the ✨ summarize plumbing; uncited findings are dropped server-side, `basis` never self-upgrades to `verified`, no `git reset` (unlike ② — this one runs on a LIVE lane that may hold uncommitted work). Built + landed *through a Fleet lane*, which is what finally broke the empty-ledger blocker. **But the first `landed` row zeroed its whole calibration payload** (`commitCount 0`, `filesTouched []`, `verified null` for a 2-commit/5-file land): `worktree.base` is a branch NAME re-resolved after landing already advanced main, and `verified` reads a `mergeLast` entry the merge route deletes before the job runs. Both are on the clean auto-land path. Diagnosis + line refs: `docs/lane-autonomy-future.md`. **Do not calibrate any gate on ledger data until the recorder fix lands.** Also this session: both model tiers moved to the 1M variants (`claude-opus-5[1m]` / `claude-sonnet-5[1m]`) — see CLAUDE.md → Deploy for the load-bearing zsh-glob quoting rule that change required.
...
| ~~P-4~~ | **SHIPPED (committed, NOT yet deployed)** — deploy-gap fact: the server stamps its own HEAD at boot, and `/api/steward/sessions` + `/api/steward/digest` each serve `deployGap {bootHead, head, behindCount, codeBehind}` route-computed per read (unknown = `null`, never a false "deployed"). Client rendering still open | — | owner: deploy | `server.ts` `deployGap()`; dossier F6 |
...
| ~~P-9~~ | **SHIPPED (2026-08-04), MOVED (2026-08-05)** — the dispatcher stopped injecting raw task text; `laneOwnerPrompts` derives `briefHash` from the first owner/auto prompt, so a dispatched lane's provenance stops rounding to `null` (`ownerPrompts` stays owner-only — attention proxy). Since 2026-08-05 the compile happens in `tickAnalysisSweep`, not on the spawn path: the brief is stored on the task (`Task.brief`), shown and editable before release, and `briefAndSend` sends those bytes with no model call. The `briefPayload` half was dropped as an illusion — a fresh lane is 0/0/clean, so that DATA block was always empty. Proven in `e2e/tasks.ts` (d) byte-equality with the stored brief, and (h3) for the analyst-compiled path | — | done | knowledge-layers §4, `docs/queue-analyst.md` |
