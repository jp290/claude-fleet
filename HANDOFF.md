...
1. Nach dem Compact: `./state.sh`, `./register.sh`, `GET /api/self` (Watches), Monitore pruefen (`ps -o pid,stat -p $(pgrep -f mon-events.py)`), `mon-events.py` neu starten, falls beendet. DANN §2b (Private-repo-j-Prompt) — das ist die Hauptarbeit der naechsten Stunde.
