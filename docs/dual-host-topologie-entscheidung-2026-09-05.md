...
*(Überholt seit W5d T1 — dort wurde genau dieser Fall zum Fehlschlag: `hub-unreachable`, grünes Gate, kein Land; gemessen 2026-09-24 an `claude-fleet-demo`. Stand seit Zeile 5857e934: ein Repo ohne Remote unter dem Namen aus `FLEET_HUB_REMOTE` hat keine Nabe — `server.ts#hubRemoteMissing` lässt `pushLandToHub` `null` antworten wie bei leerer Variable, der Land läuft lokal wie vor W5d, die Note trägt statt `hubPush` ein `hubSkipped {remote, reason:"no-remote"}`, der Trail eine `hub_skip`-Zeile. Nur ein `git remote`, das antwortete und den Namen nicht listet, zählt als fehlend; ein vorhandener, unerreichbarer Remote bleibt `hub-unreachable`. Kein Opt-out je Repo, kein Anlegen von Remotes durch den Server. Beleg: `e2e/land-durability.ts` §G4.)*
