...
> **MAIN-SELBSTANNAHME nach fuenf Astra-REJECT-Runden (19/13/14/12/10 Punkte), KEIN Astra-ACCEPT — 2026-09-07,
> Attention 58cb3e5e2546e3ed67dd554c**
>
> Eine sechste Reviewrunde wurde nicht gefahren. Alle fünf Mängellisten sind eingearbeitet; was dabei benannt statt
> geschlossen wurde, steht einzeln in **§13**. Dieses Dokument trägt keine unabhängige Abnahme.
...
`mergeJob`). **Sechster Entwurf** nach fünf Astra-Ablehnungen (19, 13, 14, 12 und 10 Punkte) und einer Opus-Gegenlesung
(19 Punkte); alle fünf Listen sind eingearbeitet und je Punkt am gepinnten Baum gegengelesen, die Kernbefunde F1–F8
haben beide Reviewer in **jeder** Runde bestätigt — die Ablehnungen trafen Fix-Skizzen, Done-Sätze, Zitate und
Überziehungen der Lesart (Runde 3: das F6-Beispiel fiel am Pin `e2e/pins.ts:398–400`, §6 trägt jetzt eines, das die
Pins überlebt; Runde 5: die Deploy-Freigaberegel gab bestätigte echte Fehler frei, §3.4 nennt sie jetzt je Verdict).
**Ein unabhängiges ACCEPT gibt es nicht** — die sechste Runde wurde nicht gefahren, das Dokument steht auf
Selbstannahme (Stempel oben, offene Punkte in §13). **Verify des Dokuments:**
...
Das Rätsel hat für **vier** seiner fünf Formulierungen eine Antwort am Code. Für die fünfte — „ein Land verliert einen
Commit" — nur in der schwächeren Lesart **Aufzeichnungsverlust**: F5 verliert B's Land-**Record** (Note, Undo-Eintrag,
Audit-Cover), nicht B's Commit aus der Git-Historie. Der Commit sitzt nach dem Boot als Vorfahr von main im Baum, nur
beschreibt ihn kein Ledger — `finishLandsInFlight` zeichnet den Marker auf, den es **findet**, und `recordLand` bekommt
`p.mainBefore` des überlebenden Markers (12760). Ein Commit, der aus der Historie verschwindet, ist hier **nicht**
nachgewiesen; die Rangliste nennt das Rätsel-Ziel entsprechend „Commit ohne Note/Undo/Audit". Die teuerste Antwort liegt
nicht in der Fehlerbehandlung, sondern in einer fehlenden **Bindung**: der Clean-Pfad von `mergeJob` verifiziert einen **Worktree-Zustand** und
...
# private suite lock, FLEET_CMD=true, fixture repo — so on the FLEET side it reaches nothing live by
# construction. On the FILESYSTEM side it makes no such promise on its own: it deletes <scratch-dir>
# recursively, and the only thing standing between that `rm -rf` and the wrong argument is the
# ISOLATION GUARD below. The guard refuses, not this comment.
#
...
# ISOLATION GUARD. A single `[ "$DIR" = "$SRC" ]` is NOT a guard for a recursive delete: another
# spelling of the same directory (symlink, trailing slash, relative path) passes that compare, and so
# does any PARENT of the checkout. Four conditions, all required, canonical paths only:
case "$DIR" in /*) ;; *) echo "refusing: <scratch-dir> must be an absolute path"; exit 9;; esac
SRC_C=$(cd "$SRC" 2>/dev/null && pwd -P) || { echo "refusing: <fleet-checkout> is not a readable directory"; exit 9; }
if [ -d "$DIR" ]; then DIR_C=$(cd "$DIR" && pwd -P)
else
  DIR_P=$(cd "$(dirname "$DIR")" 2>/dev/null && pwd -P) || { echo "refusing: the parent of <scratch-dir> does not exist"; exit 9; }
  # "/" as the parent would make "//name" — and "//name" passes the depth test below by looking
  # like two segments. Normalise, or the depth test is decorative.
  case "$DIR_P" in /) DIR_C="/$(basename "$DIR")";; *) DIR_C="$DIR_P/$(basename "$DIR")";; esac
fi
#  (1) not the checkout, not inside it — and not an ancestor of it either
case "$DIR_C" in "$SRC_C"|"$SRC_C"/*) echo "refusing: $DIR_C is the checkout or lies inside it"; exit 9;; esac
case "$SRC_C"/ in "$DIR_C"/*) echo "refusing: $DIR_C contains the checkout"; exit 9;; esac
#  (2) never a root other things live under (home roots included)
case "$DIR_C" in /|/Users|/home|/tmp|/private|/private/tmp|/var|/var/tmp|"$HOME") echo "refusing: $DIR_C is a shared root"; exit 9;; esac
#  (3) at least two path segments, so a typo cannot name a top-level directory
case "${DIR_C#/}" in */*) ;; *) echo "refusing: $DIR_C is too shallow for a scratch dir"; exit 9;; esac
#  (4) THE LOAD-BEARING ONE: either it does not exist yet, or it carries the marker file THIS script
#      wrote on an earlier run. Nothing else is ever deleted, whatever (1)-(3) say.
MARK=.p3-repro-scratch
if [ -e "$DIR_C" ]; then
  [ -f "$DIR_C/$MARK" ] || { echo "refusing: $DIR_C exists and was not created by this script"; exit 9; }
  rm -rf "$DIR_C"
fi
mkdir -p "$DIR_C" || { echo "refusing: cannot create $DIR_C"; exit 9; }
DIR="$DIR_C"; : > "$DIR/$MARK"

mkdir -p "$DIR/inst" "$DIR/repo"
...
Prozesstod im Spiel) und den exakten Spawn-Zeitpunkt (siehe oben). Und was seine Isolation **trägt**: auf der
Fleet-Seite eigener Socket, Port, Suite-Lock und ein Fixture-Repo — dort kann es die Live-Flotte strukturell nicht
erreichen; auf der **Dateisystem**-Seite allein die vier Bedingungen des Guards, von denen die vierte (existiert nicht
oder trägt die selbst geschriebene Marker-Datei) die tragende ist. Ein Pfadvergleich gegen den Checkout wäre keine:
ein `rm -rf` auf ein anders geschriebenes oder übergeordnetes Verzeichnis passiert ihn.
...
3. **Was der SHA-Pin nicht einfriert:** `runVerify` liest weiter den Arbeitsbaum. Vollständig wird die Bindung erst,
   ...
   Schritt 3:** ein Check, der das Gate **vor seiner relevanten Lesung anhält**, dann die uncommittete Datei schreibt,
   dann weiterlaufen lässt. Ohne diesen Halt beweist ein grünes Gate **nichts**: es sieht die Datei auch dann nicht, wenn
   es den relevanten Schritt längst hinter sich hat — genau die Form, die §1.3 vorführt — und mit `verify.candidateSha`
   aus Schritt 1–2 wäre die Note ohnehin schon gestempelt. Der Halt ist das Gate-Kommando selbst:
   `FLEET_VERIFY_CMD='touch "$G/started"; while [ ! -f "$G/go" ]; do sleep 0.2; done; test ! -f red.marker'` (`$G` liegt
   außerhalb beider Bäume). Der Check wartet auf `started`, schreibt `red.marker` **uncommittet** in den
   Lane-Worktree, legt `go` an und prüft danach: Gate **grün** (der `git archive <tip>`-Snapshot enthält die Datei
   nicht), `git show main:red.marker` fehlt, `main == tip`, Note `verify.candidateSha == tip`. @c7184f8 fällt derselbe
   Check an seiner **ersten** Assertion: das Gate läuft mit `cwd` = Lane-Worktree (12011–12012), sieht `red.marker` nach
   dem `go` und endet rot. Der gepinnte Advance ist dabei ausdrücklich **zulässig** — er ist `merge --ff-only <branch>`
   im Holder des Integrationsbranches (3883) und nimmt **Commits**, nie Working-Tree-Bytes; der Cleanliness-Check der
   Lane liegt **vor** dem Gate (18466), ein danach schmutziger Lane-Worktree ist für den Advance unsichtbar. Kein
   Dirty-Guard — er wäre ein zweiter Zaun um Bytes, die der Snapshot schon nicht misst.
...
Land), greift die bestehende ff-Retry-Runde (Re-Rebase + Re-Gate). Check — **die Halterichtung ist der Punkt:** der Halt muss **vor** dem Mutex-Erwerb liegen, nicht am
`FLEET_TEST_LAND_FF_LATCH`. Der steht **hinter** `markLandIntent` (18693 → 18694) und damit **im** Mutex, wo derselbe
Vorschlag jeden Undo mit 409 `land in flight` abweist — ein Undo dort kann main nicht zurückspulen, und ein Done-Satz,
der es verlangt, verlangt das Gegenteil der eigenen Serialisierung. Der Check dreht deshalb um: **der Undo hält, das
Land wartet.** `FLEET_TEST_UNDO_PAUSE_MS` (neuer Test-Seam in L2(b), Muster `LAND_PAUSE_MS` 11765/18764 — test-only,
0 in Produktion) hält den Undo unter dem Mutex an, **nachdem** er beide Tips auf `mainBefore` zurückgesetzt hat; in
dieser Pause der Merge-POST. **Eintritt bestätigen:** `GET /api/slots/<slot>/merge` steht `running`,
`rev-parse main == mainBefore` des Undos, der Undo-POST hat noch nicht geantwortet. Pause aus → Undo antwortet 200,
das Land bekommt den Mutex, liest main **neu** und findet es als Vorfahr seines `landMain` → Verdict
`error`/`main-rewound`, main unverändert auf dem Undo-Ziel, keine Note, kein Cover. @c7184f8 ist der Check in zwei
Richtungen rot: `MERGE_ERROR_REASONS` (`lane-signals.ts`, importiert 17) kennt `main-rewound` nicht, und ohne Mutex
läuft der Undo nie in eine Pause, in der ein Land wartet.
...
**rohe** Zeile ohne Urteil; das Urteil kommt getrennt aus `adjudicationsByAudit()` (15562,
`Map<auditAt, AuditAdjudication>`, newest-wins). **Die Freigabe ist je Verdict explizit**, denn eine Adjudikation ist
eine Diagnose und keine Deploy-Erlaubnis — der Quellkommentar hält genau das fest („An adjudicated red stays red BY
CONSTRUCTION", 15520). `ADJUDICATION_VERDICTS` (15536) hat vier Werte: **`flake` und `stale-test` geben frei** (jemand
hat hingesehen und dem Baum die Schuld abgesprochen), **`real` und `unknowable` blocken weiter** — `real` ist der
bestätigte Fehler, und eine Freigabe auf `unknowable` wäre die Erfolgsbehauptung über Ungemessenes, die dieses Dokument
sonst rügt —, und **keine** Adjudikation blockt ebenfalls. Also
`row.result === "red" && (adj === undefined || adj.verdict === "real" || adj.verdict === "unknowable")` mit
`adj = (await adjudicationsByAudit()).get(row.at)`. Beide Fehlformen sind damit benannt: **ohne** Join blockte (c) auch
eine als Flake beurteilte Zeile, mit einem Join **ohne** Verdict-Prüfung gäbe eine Adjudikation den Deploy auch für einen
bestätigt echten und für einen ausdrücklich unentscheidbaren Befund frei. (d) `resolveDeployMarker`
...
**Done-Sätze (je einer, `e2e/deploy-facts.ts`):** (a1) schmutziger Tree → 409 mit Dateiname; (a2) sauberer Tree, aber
ein **anderer ausgecheckter Branch** als der Integrationsbranch → 409 mit beiden Namen, kein Build, kein Marker (sonst
hat der Branch-Guard aus (a) keinen eigenen Gegencheck); (b1) Tip **ohne** Land-Note und **ohne** Override → 409
„no land note on target", kein Marker, keine Boot-Row; (b2) derselbe Tip mit `{"unmeasured":true}` → **202**
`stage:"restarting"` und die Boot-Row trägt `ok:null` mit dem Grund — der Override erlaubt den Deploy, nie ein grünes
Urteil; (c) RED-Tip in DIESEM Repo → 409, ein späteres Grün eines ANDEREN Repos hebt das nicht auf; **derselbe RED-Tip
nach einer Adjudikation `flake`** (`POST /api/post-land-audits/adjudicate {at, verdict, note}`, 10780) → **202**,
**nach `real` weiterhin 409**, **nach `unknowable` weiterhin 409** — drei Fälle auf einem Tip, und der Join mit dem
**Verdict** entscheidet, nicht die Anwesenheit einer Adjudikation. **202, nicht 200:** der Erfolgspfad antwortet
`}, 202);` (23346) mit `ok:null, stage:"restarting"`, weil der Restart diesen Prozess tötet und das Urteil der nächste
Boot schreibt; dieser Vorschlag ändert den Vertrag nicht. (d) Marker mit `at` älter als `DEPLOY_INFLIGHT_MAX_MS` beim Boot → Row `ok:null`
...
(1) `FLEET_POSTLAND_AUDIT_CMD='sh -c "echo PASS x; exit 0"'`, `FLEET_AUDIT_HELPER_GRACE_MS` hoch — **und beide
Voraussetzungen der Grace müssen hergestellt werden, sonst startet der Audit sofort und der Undo kommt zu spät.**
`graceOn` ist `AUDIT_HELPER_GRACE_MS > 0 && helperClaimCandidateExists()` (13592): ein Helfer muss enrollt und
claim-fähig sein. Und `if (entryRunsShortChain(r, q.covers)) return true;` (13608) lässt einen **proportionalen**
Eintrag die Grace unmittelbar passieren: die Lane muss ein **Code**-Land sein (Cover ohne `proportional`). Ablauf:
Code-Land, **Eintritt bestätigen** — `postLandAuditLive.waiting` nennt den Cover, `runningPostLandAudit` ist leer —,
in diesem bestätigten Wartezustand `POST /api/repos/undo-land`, dann die Grace ablaufen lassen → Zeile `green`,
`covers[0].mainAfter` kein Vorfahr von `mainSha`. Ohne den bestätigten Wartezustand misst die Skizze nichts. (2) Helfer enrollen, Job claimen (die Claim-Antwort trägt die zwölfstellige `jobId`; ohne gültige `jobId` antwortet
...
`fleet-e2e-security.ts` oder `fleet-e2e-claude-gate.ts` — von den Runnern liest `e2e/pins.ts`
`fleet-e2e-clean-review.ts` (4394/4410, zwei Strings) **und `fleet-e2e-postland-audit.ts`** (4443, ein String); die
beiden hier angegriffenen Runner nennt keiner seiner `read(…)`-Aufrufe, und `tsc --strict` meldet unerreichbaren Code ohne
...
2. **`inheritedSuiteHolder` beweist „ein lebender Prozess hält", nie „hält für mich".** 17398–17408 prüft Variable
   ...
   **Fix (L7b):** ein prozessinterner Mutex serialisiert Clean-Pfad-Gates untereinander, unabhängig von der Herkunft
   ...
   `gateInherited !== null`) → genau eines endet `exit 7` (Verdict `resolved`); mit L7b enden beide grün, und die zweite
   Note trägt Grund `same-process-gate` mit einem `suiteWait.waitMs`, das gegen die **beobachtete Restwartezeit** geprüft
   wird, nicht gegen die Laufzeit des ersten Gates: gemessen wird ab dem **jeweiligen** `askedAt` (18538–18542), das
   zweite Gate fragt um `Δ` später, und von den 15 s des ersten bleiben ihm nur `15 s − Δ`. Der Check nimmt deshalb seine
   eigenen zwei Zeitstempel — `tÜberlappung` (beide Slots melden gleichzeitig `suite:"land gate"`) und `tErsteEnde`
   (der gemeinsame Marker verschwindet) — und verlangt `waitMs ≥ (tErsteEnde − tÜberlappung) − 1000 ms`. Ein pauschales
   `waitMs ≥ 15000` würde einen **korrekt** serialisierenden Mutex verlieren, sobald das zweite Gate spät genug
   eintrifft.
...
- **Was sonst DOC_RULE-Dateien liest:** die drei Land-Gate-Wrapper und ihre Harnesse lesen keine (jede `docs/`-Nennung
  ...
  **mehreren** Stellen; die hier gelesenen sind: `AGENTS.md` (`RULE_ANCHORS` 1175, `RULE_VERIFY`), `docs/…md`-Zitate auf
  Existenz (`RULE_DOCPATH` 3156–3175), Konfigurationsmarker `<!-- pin:watchdog-spawn … -->` in Docs (1481),
  Index-Verweise in `docs/README.md` (1608), Aussagen über vorhandene Quellsymbole (1656, 1680) **und dokumentierte
  Konfigurationswerte in `docs/self-api.md`** (3673 ist der erste von mehreren `read("docs/self-api.md")`-Aufrufen,
  weitere u. a. 3708, 4072, 4185, 4534, 4992, 6438). **Diese Aufzählung ist ausdrücklich KEINE abgeschlossene
  Inventur** — sie nennt, was gelesen wurde, nicht alles, was liest; eine projektweite Negativaussage über sämtliche
  DOC_RULE-Konsumenten wird hier nicht getroffen (§12). Für F8 ist das unerheblich: **keiner** der genannten Leser prüft
  einen Seed-Anker gegen Bytes, und genau das ist die Lücke.
...
**Status dieser Reihenfolge:** ein **Vorschlag** dieses Dokuments, keine angenommene Planung. Das Dokument selbst
trägt keine unabhängige Abnahme (Stempel unter der Überschrift), und §13 nennt, was in ihm benannt statt geschlossen
ist — jede Zeile hier wird einzeln gefilt und einzeln entschieden.
...
- Gelesen durch Sub-Agents mit Nachlesung der tragenden Zitate durch die Autorin: Deploy-Verb, Undo-Route,
  Audit-Bindung, Mutex; die beiden Gegenlesungen des ersten Entwurfs haben je ~15 Zitate korrigiert, alle hier nachgezogen.

---

## 13. Offene Punkte — NICHT angenommen

Dieses Dokument trägt **kein** unabhängiges ACCEPT (Stempel unter der Überschrift). Die 68 Mängel der fünf
Astra-Runden (19 + 13 + 14 + 12 + 10) sind als exakte Ersetzungen eingearbeitet und je Punkt am gepinnten Baum
gegengelesen. Die folgenden fünf sind dabei **nicht geschlossen, sondern benannt** worden — sie stehen hier einzeln,
damit kein Leser sie für erledigt hält. Jeder ist ein *ausgewiesener Nicht-Nachweis*, keine stehengebliebene
Falschaussage; keiner liegt über „mittel".

1. **Runde 2 Punkt 1 · Runde 3 Punkt 2 — §6, L6 ist kein Integritätsnachweis.** Eingearbeitet sind der ausdrückliche
   „NICHT behoben"-Absatz und der von Runde 3 verlangte Fälschungsgegencheck (Done-Satz 4). **Rest:** L6 bleibt ein
   Mengenindikator gegen den versehentlichen Fall — ein Kandidat, der die Baseline aus `git notes --ref=fleet/land`
   liest und die passenden `PASS`- und `acquired`-Zeilen druckt, umgeht ihn, und die 80-%-Schwelle lässt bis zu 20 %
   Prüfverlust je Wrapper unbemerkt durch. Der Schnitt, der die Klasse schlösse (Prüfskripte aus `main:` gestaged),
   hat keinen Done-Satz und steht außerhalb jeder Reihenfolge (§11).

2. **Runde 1 Punkt 5 · Runde 2 Punkt 5 · Runde 3 Punkt 3 — §1.3, was Variante B beweist.** Eingearbeitet als
   ausdrücklich benannte stärkste Alternative. **Rest:** das Transkript schließt nicht aus, dass der
   Fixture-Kurzkettenlauf im veränderlichen Worktree weiterlief und ohnehin PASS gemeldet hätte. B belegt die
   **vererbte Kettenwahl** beider Stufen, nicht die Blindheit einer echten Kette gegenüber einer bestimmten Datei;
   dieser Beweis bräuchte einen Lauf der echten Kette, den dieses Dokument nicht gefahren hat.

3. **Runde 3 Punkt 12 — §12, Variante C gegen einen echten Audit.** Eingearbeitet als „offen". **Rest:** belegt ist
   allein das Rot des hier definierten Marker-Kommandos auf einem Snapshot von main. Ob die echte Audit-Suite
   (`proportional ? VERIFY_PROPORTIONAL_CMD : chosen.cmd`, 13853) denselben Baum rot gemessen hätte, ist ungemessen —
   und wird nirgends behauptet.

4. **Runde 4 Punkt 10 — §1.5, der Median von 109 s.** Eingearbeitet sind Formel (`ms − waitMs`), Filter
   (nicht-proportionale Land-Notes mit `verify.at ≥ 2026-09-01`) und Ledger-Stichtag. **Rest:** der Nenner 91 stammt
   aus einer **nicht eingefrorenen** Auswahl — dieselbe Sonde zählte beim Gegenlesen bereits 397 zeitfähige Notes —,
   und die Zeitzone des Filters steht nicht im Text. Die Zahl ist rechnerisch nachvollziehbar, ihre Reproduzierbarkeit
   Byte für Byte nicht.

5. **Runde 1 Punkt 17 — §7 Punkte 2 und 4, die Live-Prozessumgebung.** Eingearbeitet als „nicht gemessen" in §7 und
   §12. **Rest:** ob der laufende Live-Server `FLEET_SUITE_LOCK_HELD_BY` geerbt hat, wurde nicht gemessen; Zombie-
   Halter, EPERM unter fremder UID und PID-Recycling innerhalb der Birth-Auflösung von 1 s sind Argumente am Code
   ohne eine einzige Beobachtung. F7.1 und F7.3 sind davon unberührt.

**Zwei Abweichungen der Bearbeiterin, benannt statt still:**

- **Zu Runde 5 Punkt 1:** die Auftragszeile für diese Runde lautete, rot blocke, wenn *keine* Adjudikation vorliegt
  **oder** `verdict === "real"`. §3.4(c) blockt zusätzlich auf **`unknowable`**. Grund: Runde 5 verlangt negative
  Checks für `real` **und** `unknowable`, und eine Deploy-Freigabe auf einem ausdrücklich unentscheidbaren Befund wäre
  genau die Erfolgsbehauptung über Ungemessenes, die dieses Dokument sonst rügt. Die Abweichung ist konservativ in
  Blockrichtung und betrifft nur den vierten Verdict-Wert.
- **Zum Repro-Skript (§1.6):** sein Isolationsguard wurde in dieser Runde ersetzt und danach **syntaktisch geprüft
  (`sh -n`) und in seinen Ablehnungspfaden ausgeführt** — zehn Argumentformen, darunter Symlink auf den Checkout,
  übergeordnetes Verzeichnis des Checkouts, `$HOME`, `/tmp`, ein bestehendes Verzeichnis ohne Marker (Daten blieben
  unangetastet) und ein zu flacher Pfad. Die drei Varianten A/B/C wurden **nicht** erneut gefahren: die Ausgabe in
  §1.3 stammt weiter vom Lauf am 2026-09-07 13:1x, dessen Skript denselben Body und einen schwächeren Guard hatte.
  Der Guard läuft vor jeder anderen Zeile und berührt den gemessenen Ablauf nicht — bewiesen ist das nicht, benannt
  ist es hier.
