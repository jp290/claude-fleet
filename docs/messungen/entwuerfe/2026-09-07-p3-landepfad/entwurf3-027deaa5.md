...
`mergeJob`). **Dritter Entwurf** nach zwei Astra-Ablehnungen (19 und 13 Punkte) und einer Opus-Gegenlesung (19 Punkte);
alle Listen sind eingearbeitet, die Kernbefunde F1–F8 haben beide Reviewer bestätigt — die Ablehnungen trafen
Fix-Skizzen, Done-Sätze, Zitate und Überziehungen der Lesart. **Verify des Dokuments:**
...
ihn mit keinem gelandeten oder auditierten SHA, blockt nur auf einen *laufenden* Audit und schreibt nach einem
Boot-Crash-Loop `ok:true` (F3). Viertens bindet der Audit sein Urteil an `rev-parse main` **zur Laufzeit**, nicht an
...
| 3 | F3 Deploy nimmt den Working-Tree, vergleicht nichts, blockt nur `running`, Boot-Crash-Loop endet `ok:true` | ungemessener Baum live | Argument am Code, Repro-Skizze | „deployed" impliziert keine Messung |
...
| 6 | F6 Gate-Kette führt die Wrapper des Kandidaten aus — ein `exit 0` im Wrapper ist ein grünes Gate | Verify-Urteil fälschbar | Argument am Code (Designentscheid) | jede Lane kann sich selbst grün schalten |
...
- **B** (docs-only, Docs→Code): main == C2; Note `proportional:true, steps [install,pins]`; die Audit-Zeile für genau
  ...
  nicht sieht — die Fixture-Kette ist inhaltsblind, das echte `pins.ts` gegenüber `server.ts` ebenfalls (§1.2, Argument).
- **C** (voll, inhaltssensitiv): das Gate bewertete den Baum ohne `red.marker` grün; C2 fügt `red.marker` hinzu;
  ...
  Rätsels, gemessen — und der Audit-Stand-in dazu ist grün, weil er ein Stand-in ist; ein echter Audit hätte hier
  gemessen, aber erst nach dem Land.
...
Ein Commit, dessen **Committer-Zeit** danach liegt, wurde nach diesem Zeitpunkt erzeugt (die Pre-Pass-Rebase liegt
strukturell davor). Ergebnis: **7 von 396** Lands mit Zeitinformation; alle 7 sind **Confirm-Lands**
...
Live-Ledger** — aber die Sonde ist nur an der Committer-Zeit möglich, weil die Note den Lane-Tip zur Gate-Zeit nicht
...
- Exponierte Menge live: 60 von 482 Notes fahren die Kurzkette (`proportional:true`) — eine Größe der Klasse, kein
  Maß des Fensters. Das Fenster ist die Gate-Laufzeit: ~1 s Kurzkette, ~110 s volle Kette (Median, Land-Notes seit
  01.09.), plus bis zu 45 min Schlange — in der Schlange darf die Lane-Session frei committen, weil der Idle-Gate der
...
Exit 0 heißt: alle sechs Assertions hielten (drei Notes, roter Baum in C, Kurzkette in B's Audit-Zeile). Bei behobenem
...
3. **Was der SHA-Pin nicht einfriert:** `runVerify` liest weiter den Arbeitsbaum. Vollständig wird die Bindung erst,
   ...
   machen würde, und beweist, dass das Gate trotzdem grün bleibt (Snapshot) und die Datei nicht landet (dirty → kein ff).
...
landet docs-only, committet Code während der Kurzkette und beweist, dass die Audit-Zeile die **volle** Kette nennt
oder der Land abgelehnt wurde. Das Skript in §1.6 endet mit Exit 1 und dreimal `main != C2`.
...
- Zusätzlich eskaliert die Route **jede** nicht-null-Antwort von `remoteHoldsLandedRange` gleich, auch die beiden
  fail-closed-Zweige „cannot list …"/„cannot tell …" (12460, 12462) — ein `index.lock`-Treffer oder ein
  `GIT_TIMEOUT_MS`-Ablauf unter Suitenlast löscht bis zu drei Rollback-Zeiger, bevor irgendetwas bewegt wurde. Der
...
`markLandIntent…recordLand` schützt — ein Land, das in dieses Fenster fällt, wartet oder wird abgewiesen) und in dieser
Reihenfolge: (1) Hub zurück, **atomar mit Erwartungswert**: `git push <hub>
...
Land, das während des Undos advanct (`FLEET_TEST_LAND_PAUSE_MS` auf dem Land, Undo-POST in der Pause): genau einer der
beiden Züge wird abgewiesen, main steht danach auf einem SHA, den eine Note oder ein Undo-Eintrag beschreibt — nie auf
einem, den keins von beiden kennt. Für (c): ein Check,
...
- `deployRun` (23294): Blocker (23295) → Marker-Alter (23297–23308) → **eine** git-Lesung `rev-parse HEAD` (23310–23314,
  `target`) → Build (`sh -c $FLEET_DEPLOY_BUILD_CMD`, cwd = Haupt-Checkout, 23319) → Marker → Restart. Kein
...
- `judgeDeploy` (23161): `ok` beginnt bei `true` ohne `extra` (23167) und fällt nur auf `bootHead !== head` oder
  `bundle.stale`. `bundleStale` (23024–23031) vergleicht **Mtimes** von `public/*.js` gegen `src/` — kein Inhalts-,
...
- `resolveDeployMarker` (23198): kein Alters-/Lebendigkeits-Check; `DEPLOY_INFLIGHT_MAX_MS` (23083) gilt nur im
  ...
  `claimInstanceLock` 21171/21185, `tmux start-server` 21205, `finishLandsInFlight` 21833,
  `recoverInterruptedProgramFoundings` 21871), respawnt `watchdog.sh:111-164` alle 5 s; der erste Boot, der
  durchkommt, schreibt `stage:"boot", ok:true` — die Zeit im Crash-Loop steht nur in `ms`, das kein Konsument liest.
...
ein späteres Audit eines anderen Repos überschreibt), sondern über `newestAuditFor(repo, target)` (11128):
`result === "red" && !adjudicated`. (d) `resolveDeployMarker`
...
(`m.at` wird nach dem Build gesetzt, 23337) — das Alter ist messbar, die Ursache nicht. (e) `deployBlocker` wird **nach** dem Build erneut gefragt; Treffer →
Marker nicht schreiben, 409 mit Grund.

**Done-Sätze (je einer, `e2e/deploy-facts.ts`):** (a) schmutziger Tree → 409 mit Dateiname; (b) Tip ohne Note →
Boot-Row `ok:null`; (c) RED-Tip in DIESEM Repo → 409, ein späteres Grün eines ANDEREN Repos hebt das nicht auf; (d) Marker mit `at` älter als `DEPLOY_INFLIGHT_MAX_MS` beim Boot → Row `ok:null`
mit dem Grund; (e) Land während des Builds (Build = `sleep`, Merge-POST im Fenster) → kein Restart, 409, Verdict des
Lands normal terminal.
...
`covers[0].mainAfter` kein Vorfahr von `mainSha`. (2) Helfer enrollen, Job claimen, Bundle **nicht** laden,
`POST /api/helper/result {exitCode:0}` ohne `clonedSha` → grüne Zeile ohne `remote.clonedSha`; mit `clonedSha:"f"*40`
→ Event-Payload byteidentisch zum passenden Fall. (3) `POSTLAND_AUDIT_FILE` in ein nicht existierendes
Elternverzeichnis zeigen lassen → `appendEvent` scheitert still, Queue-Datei ist verbraucht, keine Zeile — lokal wie
remote. (4) `FLEET_POSTLAND_AUDIT_CMD='sh -c "sleep 1; exit 0"'` → `{green, checks:{ran:0}}`.
...
time" voraus, nichts erzwingt es). Nicht reproduziert. **Repro-Skizze, deterministisch über `FLEET_TEST_LAND_PAUSE_MS`** (die Pause liegt **nach**
`advanceIntegration` und **vor** `recordLand`, 18764 — genau das Fenster): Scratch mit
`FLEET_TEST_LAND_PAUSE_MS=30000`. Lane A (Code, volle Kette = `sleep 20`) landen; 2 s später Lane B (docs-only,
Kurzkette ~1 s) landen — B's `markLandIntent` überschreibt A's Marker (B ist schneller am Marker, weil seine Kette
kürzer ist), B advanct zuerst, A's Advance verliert den ff und geht in die Retry-Runde, B pausiert 30 s. Variante ohne
Race-Abhängigkeit: A allein landen, in A's Pause **von Hand** `markLandIntent` nachstellen ist nicht möglich — also
die Reihenfolge umdrehen: Lane B (docs) zuerst mit `LAND_PAUSE_MS=30000` → B advanct und pausiert; **in der Pause**
Lane A (Code) landen: A's `markLandIntent` überschreibt B's Marker, A's `advanceIntegration` fast-forwardet auf A's Tip
(Kette: B-Tip → A-Tip); jetzt `tmux kill-session -t srv` der Scratch-Instanz, bevor B's Pause endet. Boot: der Marker
ist A's (`mainBefore` = B-Tip, `laneTip` = A-Tip), main steht auf A-Tip → `finishLandsInFlight` **zeichnet A auf**
(zweiter Zweig) — und **B's Land ist unrecorded**: main hat sich von B's `mainBefore` wegbewegt, B's Marker existiert
nicht mehr, keine Note auf B-Tip, kein Undo-Eintrag für B, keine Audit-Zeile mit B's Cover. Das ist der Verlust,
kontrolliert erzeugt.
...
record"; besser: ein Repo-weiter Land-Mutex um `markLandIntent…recordLand` (Sekunden, kein Suite-Lock). **Done-Satz:**
Check in `e2e/land-durability.ts` nach der Skizze oben (B pausiert nach Advance, A landet in der Pause, Kill, Boot):
**jeder** Commit in `rev-list <main-vor-B>..<main-nach-Boot>` liegt in genau einer Land-Note und in genau einem
Undo-Eintrag (oder das zweite Land wurde vor seinem Advance abgewiesen und main steht auf B-Tip mit B's Note). Ein
...
Kandidaten**. Ein Commit, der `e2e-security.sh` auf `exit 0` setzt oder einen `check()` entfernt, ist ein grünes Gate;
der Post-Land-Audit fährt `./e2e-isolated.sh` aus dem Snapshot **desselben** Baums (13890, `cwd: dir`). Beide Stufen
messen also, was der Kandidat zu messen erlaubt. Das ist ein Designentscheid (Tests müssen den Kandidaten testen) und
...
`grep -c 'check('` in `pins.ts` — das findet dort nichts, weil Pins `pin(` heißen. **Done-Sätze:** (1) Check in
`e2e/merge.ts`: erst ein volles grünes Land (setzt die Baseline), dann eine Lane, die `e2e-security.sh` durch `exit 0`
ersetzt → Verdict `resolved` mit `checksBelowMain`, kein Land; (2) Kontrollgruppe: unveränderte Wrapper → kein Stempel,
`passLines` je Wrapper > 0; (3) ein docs-only-Land dazwischen ändert die Baseline nicht (die proportionale Note wird
übersprungen).
...
2. **`inheritedSuiteHolder` beweist „ein lebender Prozess hält", nie „hält für mich".** 17398–17408 prüft Variable
   ...
   **Vorschlag (unter der Schnittlinie, kein Done-Satz):** ein prozessinterner Mutex serialisiert Clean-Pfad-Gates
   untereinander, unabhängig von der Herkunft des Holds.
3. **Die ff-Retry-Absage schreibt eine Wartezeit, die nie verging.** Zweiter `mergeJob` im selben Prozess, während
   ...
   **Vorschlag (unter der Schnittlinie, kein Done-Satz):** die Absage nennt die gemessene Dauer und den Grund
   `same-process-hold` getrennt.
...
- **Offen, Antwort 2 — Anker in DOC_RULE-Dateien, die nur die volle Suite prüft (F8).** Sieben Context-Pack-Anker
  ...
  gefangen, für `AGENTS.md`-Zitate von `RULE_ANCHORS` (1175); die Lücke sind Überschriften-Renames und
...
- **`attic/` als DOC_RULE** hält `attic/steward-arena.sh`, das `e2e-stage.sh` sourct (Zeile 6) und für dessen
...
- **Was sonst DOC_RULE-Dateien liest:** die drei Land-Gate-Wrapper und ihre Harnesse lesen keine (jede `docs/`-Nennung
  dort ist Kommentar); `tsc` hat eine feste Liste, `build` bündelt `src/`. `e2e/pins.ts` liest `AGENTS.md`
  (`RULE_ANCHORS`, `RULE_VERIFY`) und prüft `docs/…md`-Zitate auf Existenz (`RULE_DOCPATH`) — das ist gedeckt.
...
- F7.2/F7.3 als Vorschläge ohne Done-Satz (§7).
...
- **P2 (Adressierbarkeit):** Audit-Events (`auditEventPayload`, 5660) tragen weder `clonedSha` noch eine
  ...
  `binding:{ancestry:"proven"|"orphaned"|"unknown", clonedSha?}` im Event.
...
5. **L6** `checksBelowMain` (§6), optional nach L1.
...
Unter der Schnittlinie ohne Done-Satz: L7b, L7c.

## 12. Nicht gemessen / nicht gelesen

- Nicht gefahren: F2–F8-Repros (Skizzen); keine Suite, kein Live-Server, kein Deploy, kein Undo.
- Nicht gemessen: Häufigkeit von F1 auf der Live-Flotte (Sonde §1.4 ist eine Untergrenze); ob zwei Lands desselben
  ...
  Undo-Versuche (F2); ob ein echter Audit (statt des Stand-ins) den Variante-C-Baum rot gemessen hätte (er hätte;
  Argument: derselbe Baum, dasselbe Gate-Kommando — aber nicht gefahren).
...
- Grenzen des Repro-Skripts (§1.6): es prüft keinen Prozesstod und keine Kill-Reihenfolge (kein Kill im Spiel), es
  ...
  keine vorhandene Zahl. L7b/L7c und der Event-Feld-Vorschlag (§10) haben keine Done-Sätze und stehen unter der
  Schnittlinie.
