...
`mergeJob`). **Vierter Entwurf** nach drei Astra-Ablehnungen (19, 13 und 14 Punkte) und einer Opus-Gegenlesung (19 Punkte);
...
prüft Schritt 4 **unabhängig von Schritt 1–2 und ohne Rennen**: ein docs-only-Land (Cover `proportional:true`), dann —
bevor der Audit startet (Cover wartet in der Grace, `FLEET_AUDIT_HELPER_GRACE_MS` hoch) — ein **direkter** Code-Commit
auf main (kein Land, kein Cover; `state.sh` zählt 31 solche Commits im Live-Ledger); die Audit-Zeile für den Tip muss
die **volle** Kette nennen (`proportional` fehlt, `cmdSource !== "proportional"`). @c7184f8 wählt `entryRunsShortChain`
(12879) allein über die Cover-Stempel → Kurzkette → der Check ist heute rot und wird durch Schritt 4 grün; Schritt 1–2
ändern an ihm nichts, weil kein Commit während eines Gates fällt. Das Skript in §1.6 endet mit Exit 1 und dreimal
`main != C2`.
...
(Undo hält den Mutex, Land-POST trifft ein): das Land wartet, seine Note trägt `mainBefore` = Tip **nach** dem Undo.
In beiden Fällen steht main nie auf einem SHA, den weder Note noch Undo-Eintrag beschreibt. Für (c): ein Check,
...
Kandidaten**. Die **Wrapper** sind gepinnt und kein brauchbarer Angriffspunkt: `e2e/pins.ts:369` verlangt je Wrapper
den Runner-Aufruf `bun fleet-e2e-*.ts` und als letzte Zeile `exit $code` (der Decapitation-Pin aus `613faa3`), 398–400
die PORT-Deklaration aus der Band-Tabelle (`e2e-isolated.sh:33`), 338–339 das Paar `stage_instance`/`. e2e-stage.sh`
— ein Wrapper-Ersatz durch `exit 0` fällt schon an Stufe 1. Die **Runner** sind es nicht: kein Pin liest
...
`./e2e-isolated.sh` aus dem Snapshot **desselben** Baums (13890, `cwd: dir`) und damit dieselben Runner. Beide Stufen
...
2. **`inheritedSuiteHolder` beweist „ein lebender Prozess hält", nie „hält für mich".** 17398–17408 prüft Variable
   ...
   **Fix (L7b):** ein prozessinterner Mutex serialisiert Clean-Pfad-Gates untereinander, unabhängig von der Herkunft
   ...
   `pid`/`birth` des Wrappers, `FLEET_SUITE_LOCK_HOLDER` in der Server-Env), Gate-Kommando = ein Shell-Einzeiler, der
   einen **gemeinsamen** Busy-Marker prüft (`exit 7`, wenn er schon existiert), ihn anlegt, 5 s schläft und ihn
   entfernt; zwei Lanes mit Code-Diff gleichzeitig landen: @c7184f8 laufen beide Gates parallel (18536 je Job `gateInherited !== null`)
   → eines endet `exit 7` (Verdict `resolved`); mit L7b enden beide grün und die zweite Note trägt `suiteWait.waitMs ≥
   5000` mit Grund `same-process-gate`.
