...
- `codex-quota.ts` — bun codex-quota.ts [--json] — quota snapshot and unseen reset announcements.
...
- `land-log.ts` — Land-Chronik — one line per land, read off the fleet/land notes that already exist.
- `land-quality.ts` — The delayed truth about a land — were the lines it inserted rewritten by main within days?
- `lane-context-cost.ts` — What a lane's grounding costs before its first productive step, per brief cohort — read off the transcripts.
- `lane-preview.sh` — lane-preview.sh — a TIME-BOXED, ISOLATED test instance of ONE parked review candidate, so the
...
- `scratch-reap.sh` — scratch-reap.sh — the owner of the e2e-isolated scratch heap.
...
- `simulator-hygiene.ts` — simulator-hygiene.ts — may an idle iOS Simulator be shut down? The pure decision, and the probe it reads.
...
- `suite-modules.ts` — WHICH CHECK MODULES A LANE MAY PREVIEW ALONE — the fixture map behind FLEET_E2E_MODULES.
...
- `testinstanz.sh` — A STANDING test instance of this checkout, for looking at the left bar with a browser.
- `titlebytes.ts` — Is a stream increment NOTHING BUT window-title updates? Pure so the fixtures in e2e/slots.ts pin
- `trailstats.ts` — The flake question — answered from the trail instead of from a seven-minute re-run.
- `variant-compare.ts` — T4 · THE VARIANT COMPARISON — the LEDGER ROW and the STAGE RULE, pure (server.ts#tickVariantCompare
- `verify-proportion.ts` — Local proof is deliberately conservative: an empty or unfamiliar footprint expands to the
- `waits.ts` — THE WAIT REGISTER — every row of the start plan that does not start now, with WHY it waits and
