...
- **Sense** (read-reduced, capability-asymmetric by design): `GET /api/steward/sessions`
  (server.ts, grep `/api/steward/sessions`) → per-slot `id, cwd, label, lastOutput, git{branch,dirty,ahead,behind},
...
- **Nudge** (the only way it moves work): `POST /api/steward/send` — *fixed
  ...
  currently-true deterministic fact; now gated through the shared `canDeliver` (master stop + quiet hours + fresh alive + idle 60s), then hourly cap (10), 1 per
  kind×slot per 10-min episode. Caps are re-derived from the durable `audit.jsonl`, not
  memory (`stewardRecentSends()`).
...
`tickAutos` fires every 5s (`tickAutos()` in server.ts): **global kill-switch first** (`autosOn` — mutes the entire *scheduled* surface, all autos incl. the live `/rundgang` beat, by
not ticking) → skip if `now < nextAt` → then the shared `canDeliver(s, opts)` choke-point:
**fresh claude-alive** → **quiet hours** (recurring autos only,
tick-in-place reschedule; one-shots always fire — `inQuietHours`) → **idle gate**
(`idleSec` vs `lastOutput`) → fire (`sendText`) →
`advanceAuto` (perpetual **re-arms without decrementing**; non-perpetual burns `runsLeft` →
dies at 0). **The former seam is CLOSED (2026-07-22):** the master stop + quiet-hours +
a fresh alive-check now reach a direct `/api/steward/send` AND the dispatcher (pre-gate + requeue-on-fail re-gate, both in `tickDispatch`) through the *same* `canDeliver`, not just `tickAutos` —
...
`POST /api/slots/:id/autos`). *The live beat:* `/rundgang` every 7200s, idle-gated 60s,
...
2. **CLOSED — `claudeAlive` (computed by `claudeAlive()` in server.ts) is on the steward read routes** as the
...
3. **CLOSED — the FULL `mergeLast` verdict** ({status, detail, conflicted, at} via
   `stewardMergeView()` in server.ts) is on the steward sessions + brief routes — a lane whose
...
5. **CLOSED — Task / `laneTask` status** rides on the steward surface (`stewardTaskView()` in
   server.ts: id/status/source/text) — the founding-intent baseline against which
...
- **Orchestration / outsourcing to fresh Opus/Fable sessions.** Fleet *already is* a
  ...
  are interactive `claude`, never metered `-p` (designed so, `slotCmd()` in server.ts). **The
