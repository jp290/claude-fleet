...
# below inherits this env and tmux bakes it into EVERY pane it spawns. That contaminates what
# e2e/self-token.ts probes: a pane would report the OUTER lane's credential as if the test server
# had put it there. Strip the ambient creds so the test server's OWN selfExport logic (server.ts)
# is the only thing that can put a self-token into a pane. Still required after 2026-08-07, when
...
# e2e/ctl.ts used to reach it through the node_modules pointer home instead, and that answer only
# exists when the source tree is a git work
...
# THE DAEMON-VINTAGE FLOOR for this instance. The server refuses a command-job claim whose device
# reports a daemonSha that does not have the command-kind commit as an ancestor, and it measures
# that in FLEET_HELPER_UPDATE_REPO (default: its own checkout — here, $DIR). The real floor is a
# commit of the SOURCE repo, which $DIR has never heard of, so an unset knob would make the guard
# refuse every claim in this instance and the refusal would say nothing about the code under test.
# Pointed at $DIR's own HEAD instead: e2e/watch.ts enrolls its command-job device with exactly that
# sha, and `--is-ancestor X X` is true.
FLOOR_SHA="$( cd "$DIR" && git rev-parse HEAD )"
...
int main(int argc, char **argv) {
  ...
  if (ox && !has_arg(argc, argv, "--no-approve")) {
    fputs("Trust project folder?\n", stdout);
      ...
      && has_arg(argc, argv, "--no-skills")
      && has_arg(argc, argv, "--no-prompt-templates")
      && has_arg(argc, argv, "--no-themes")
...
# nothing is due, and no analyst exists any more (retired 2026-09-10), so the git read inside
...
# THE FOUR SERVER WAIT WINDOWS. All four are env knobs whose UNSET value is the production literal
# (pinned in e2e/pins.ts, where no suite env reaches); the suite shortens them because a wait window
# is wall clock this suite spends NOT measuring. Sized off the phase measurement of 2026-09-14
# (docs/messungen/2026-09-14-suite-wartezeit-phasen.md), which booked ~530 s of a 2 225 s run behind
# exactly these four numbers. Anything that reads them back — e2e/lane-helpers.ts, e2e/security.ts,
# e2e/programs.ts' unbound-succession fixture — parses the SAME env with the SAME floor, so the
# harness window and the server gate can never be two numbers.
# FLEET_MERGE_IDLE_MS=500 shortens the land/commit idle gate (production 3000). Nearly every land in
# this suite first out-waits the fresh lane pane's own shell prompt, so the gate's width was paid
# once per land (~100 lands): 145,4 s in `settleForMerge` alone. 500 is the server's own floor.
# e2e/lanes-basic.ts's busy probes no longer BORROW their window from the gate — at 500 a fixed 1 s
# margin below it is a window that cannot open — they MAKE one with a bounded output generator and
# bracket the POST with two busy reads, which is a guard the gate's width cannot weaken.
# FLEET_GIT_TICK_MS=2000 shortens the git/liveness fact cache (production 10 000). Every check that
# waits for `git.ahead` or a liveness reading waits for a tick it cannot trigger; 190,7 s of that
# run sat in `e2e/programs.ts#waitDoneLooking` and 33,0 s in `e2e/security.ts#agentOf`, which sleeps
...
# FLEET_DEVICE_ONLINE_MS=20000 makes the helper-presence TRANSITION observable. The production
# default is 90 s (server.ts#DEVICE_ONLINE_MS) and the offer door refuses to mint a preview offer
# while nothing is beating inside that window — so with the default, e2e/lane-suite.ts could only
# ever measure the online half: e2e/watch.ts enrols a device minutes earlier in the same run, and
# 90 s is long enough that "is it stale yet?" would be a race rather than a fact. 20 s is the
# smallest value with real headroom in BOTH directions: the module beats immediately before every
# mint (one localhost round trip, milliseconds, ~1000x margin), while the gap between watch.ts's
# beat and lane-suite's first call is nine intervening modules of lane opens and lands. It is a
# policy number, not a test-only knob — an operator whose daemon beats every five minutes needs a
# wider window for the same reason this suite needs a narrower one.
# FLEET_BRIEF_MS=0 is stated for the same reason and one worse consequence: the brief compiler's
...
# FLEET_CARD_MS=0 stands beside it for exactly that reason: the card extractor's tick spawns a
# throwaway Haiku per open row, and §(j2) arms it with a stand-in and restores it to THIS value.
