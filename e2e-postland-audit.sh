...
for _s in "$TMUX_SOCKDIR"/fleetplatest*; do
  [ -S "$_s" ] || continue
  _own="${_s##*/fleetplatest}"
  case "$_own" in ''|*[!0-9]*) continue ;; esac
  [ "$_own" = "$$" ] && continue
  kill -0 "$_own" 2>/dev/null && continue
  tmux -L "fleetplatest$_own" kill-server 2>/dev/null
done
...
if [ "$code" = "000" ]; then
  stage_server_start_failed "e2e-postland-audit.sh" "post-land audit harness" "$DIR"
  exit 3
fi
sleep 0.5
...
FLEET_E2E_SUITE=postland-audit \
  ...
  FLEET_PORT=$PORT FLEET_SOCK=$SOCK FLEET_AUTO_REVIEW_MS=0 FLEET_BRIEF_MS=0 FLEET_CARD_MS=0 FLEET_CMD=true \
