...
tmux -L "$SOCK" new-session -d -s srv \
  "cd '$DIR' && FLEET_HOST=127.0.0.1 FLEET_PORT=$PORT FLEET_SOCK=$SOCK FLEET_TOKEN=$TOKEN \
   ...
   FLEET_CMD=true FLEET_AUTO_REVIEW_MS=0 FLEET_ANALYSIS_MS=0 FLEET_BRIEF_MS=0 FLEET_INTAKE_SECRET=$INTAKE \
...
if [ "$code" = "000" ]; then
  stage_server_start_failed "e2e-security.sh" "security harness" "$DIR"
  exit 3
fi
sleep 0.5
