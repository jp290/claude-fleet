...
export async function run(lc: LaneCtx): Promise<void> {
  ...
  // --- THE CLASS the two checks above are symptoms of: /api/slots/:id/worktrees is served from a
  // stale-while-revalidate cache that ages on GIT_TICK_MS, but three of the fields it serves are
  // not git-derived at all — `main` (configured integration branch), `slot` (holding session) and
  // `note` (shelve note). A mutation to any of those must be visible in the NEXT request, not
  ...
  // that instead, and fail for a reason this probe is not about.)
  {
    ...
    // caching — it must fail as ITSELF, never pass as a repaired overlay
    const elapsed = Date.now() - t0;
    check("overlay probe: the second read landed inside the freshness window", elapsed < tick, `${elapsed}ms of ${tick}ms`);
    check("non-git field is fresh on a cache hit: main", after.main === "integ-overlay-decoy", after.main);
