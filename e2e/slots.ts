...
export async function run(): Promise<void> {
  ...
  const composerSlot = 3;
  const composerPane = `s${composerSlot}`;
  const composerOpen = await post(`/api/slots/${composerSlot}/open`, { cwd: "~" });
  check("delivery header composer fixture opens its own claude slot", composerOpen.ok, `${composerOpen.status}`);
  if (composerOpen.ok) {
    try {
      await tmuxOut("send-keys", "-t", composerPane, "cat", "Enter");
      for (let i = 0; i < 50; i++) {
        if ((await tmuxOut("display-message", "-p", "-t", composerPane, "#{pane_current_command}")).out.trim() === "cat") break;
        await Bun.sleep(100);
      }
      const helloSend = await post("/send", { slot: composerSlot, text: "hallo", submit: false });
      await Bun.sleep(400);
      const helloCapture = await tmuxOut("capture-pane", "-t", composerPane, "-p", "-J");
      const helloLine = helloCapture.out.split("\n").find((line) => line.endsWith("] hallo"));
      await tmuxOut("send-keys", "-t", composerPane, "Enter");
      const slashSend = await post("/send", { slot: composerSlot, text: "/effort high", submit: false });
      await Bun.sleep(400);
      const slashCapture = await tmuxOut("capture-pane", "-t", composerPane, "-p", "-J");
      ...
      check("delivery header: owner /send heads hallo but keeps /effort high first in the claude composer",
        helloSend.ok && !!helloLine?.includes(`[fleet-zustellung · POST /send mit Owner-Credential · path=owner · Slot ${composerSlot}]`)
          && slashSend.ok && slashLine?.trim() === "/effort high",
        JSON.stringify({ hello: helloLine?.slice(0, 160) ?? null, slash: slashLine?.slice(0, 160) ?? null }));
    } finally {
      await post(`/api/slots/${composerSlot}/kill`, {});
    }
  }
