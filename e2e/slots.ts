...
export async function run(): Promise<void> {
  ...
  // --- The slot row redesign is a placement contract, not a DOM snapshot. Lift the pure
  // slotRowFields() decision out exactly like stacksOf() above and execute it over fixtures. This
  // closes the old failure mode where a visual rewrite silently lost a signal while still building.
  {
    const a = cliSrc.indexOf("const SLOT_ROW_DROPPED");
    const b = cliSrc.indexOf("function updateTitle(", a);
    const fieldSrc = a >= 0 && b > a ? cliSrc.slice(a, b) : "";
    type FieldPlan = { row: string[]; detail: string[] };
    type FieldFn = (slot: Record<string, unknown>, stack: Record<string, unknown> | undefined, now: number) => FieldPlan;
    let slotRowFields: FieldFn | null = null;
    let slotContextText: ((slot: Record<string, unknown>) => string) | null = null;
    let branchTail: ((branch: string) => string) | null = null;
    let dropped: Record<string, string> | null = null;
    try {
      const lifted = new Function(new Bun.Transpiler({ loader: "ts" }).transformSync(fieldSrc)
        + "\nreturn { slotRowFields, slotContextText, branchTail, SLOT_ROW_DROPPED };")() as
        { slotRowFields: FieldFn; slotContextText: (slot: Record<string, unknown>) => string;
          branchTail: (branch: string) => string;
          SLOT_ROW_DROPPED: Record<string, string> };
      slotRowFields = lifted.slotRowFields;
      slotContextText = lifted.slotContextText;
      branchTail = lifted.branchTail;
      dropped = lifted.SLOT_ROW_DROPPED;
    } catch { /* the named probe below must fail as itself */ }

    const repo = "/repo/one";
    const lane = { id: 9, cwd: `${repo}/wt/260810-tail`, lastOutput: 90, label: null, autoActive: false,
      worktree: { repo, branch: "fleet/260810-tail" },
      git: { branch: "fleet/260810-tail", dirty: 2, ahead: 0, behind: 1 }, ctx: null };
    const main = { id: 4, cwd: repo, lastOutput: 100, label: "main", autoActive: true,
      git: { branch: "main", dirty: 1, ahead: 2, behind: 3 },
      share: { comments: 2 }, mergePending: true,
      container: "box", containerContext: "ctx", ctx: { pct: 47 } };
    const stack = { key: repo, anchor: main, lanes: [{ ...lane, share: { comments: 1 }, mergePending: true }],
      at: 4, open: false };
    let mainPlan: FieldPlan | null = null;
    let lanePlan: FieldPlan | null = null;
    try {
      mainPlan = slotRowFields?.(main, stack, 1000) ?? null;
      lanePlan = slotRowFields?.(lane, undefined, 1000) ?? null;
    } catch { mainPlan = null; lanePlan = null; }
    check("probe: slotRowFields is liftable out of src/client.ts and both row forms build",
      !!slotRowFields && !!slotContextText && !!branchTail && !!dropped && !!mainPlan && !!lanePlan,
      fieldSrc.slice(0, 80) || "no SLOT_ROW_DROPPED…updateTitle block in src/client.ts");

    const TODAY = ["n", "lbl", "autobadge", "stackfold", "stackn", "revb-agg", "cmtb-agg",
      "quicklane", "lanechip", "lcdot", "lanediff", "cmtb", "revb", "ctxfill", "act"];
    const classified = [...(mainPlan?.row ?? []), ...(mainPlan?.detail ?? []), ...Object.keys(dropped ?? {})];
    const counts = Object.fromEntries(TODAY.map((id) => [id, classified.filter((x) => x === id).length]));
    check("slot row completeness: every one of today's 15 signals is row, detail or DROPPED exactly once",
      TODAY.every((id) => counts[id] === 1), JSON.stringify(counts));
    check("slot row completeness: every DROPPED signal carries a non-empty reason",
      Object.keys(dropped ?? {}).length === 0
        || Object.values(dropped ?? {}).every((reason) => typeof reason === "string" && reason.trim().length > 0),
      JSON.stringify(dropped));

    const MAIN_T1 = ["n", "lbl", "ctxfill", "branchbtn", "commit-light", "work-light"];
    const mainCore = (mainPlan?.row ?? []).filter((id) => MAIN_T1.includes(id));
    check("slot row T1: a main fixture has the six ordered core fields including its number",
      JSON.stringify(mainCore) === JSON.stringify(MAIN_T1), JSON.stringify(mainPlan));
    const laneCore = (lanePlan?.row ?? []).filter((id) =>
      ["branch-ref", "lbl", "ctxfill", "commit-light", "work-light"].includes(id));
    check("slot row T1: a lane fixture is branch-led, unnumbered, and keeps its number in detail only",
      JSON.stringify(laneCore) === JSON.stringify(["branch-ref", "ctxfill", "commit-light", "work-light"])
      && !lanePlan?.row.includes("n") && lanePlan?.detail.includes("n") === true,
      JSON.stringify(lanePlan));
    check("slot row T1: lane identity drops fleet/ and preserves the identifying branch tail",
      branchTail?.("fleet/12345678901234567890-identifying-tail") === "…90-identifying-tail",
      String(branchTail?.("fleet/12345678901234567890-identifying-tail")));
    check("slot row T2: container detail exists exactly when the poll carries the resolved pair",
      mainPlan?.detail.includes("container") === true && lanePlan?.detail.includes("container") === false,
      JSON.stringify({ main: mainPlan?.detail, lane: lanePlan?.detail }));
    check("slot row T3: null context is unknown text, distinct from a measured zero percent",
      slotContextText?.({ ctx: null }) === "ctx ?" && slotContextText?.({ ctx: { pct: 0 } }) === "ctx 0%",
      `${slotContextText?.({ ctx: null })} / ${slotContextText?.({ ctx: { pct: 0 } })}`);
    check("client: active lane rows render only through their stack anchor and both row forms retain project tint",
      /if \(g && s\.worktree\) continue/.test(cliSrc)
      && /function slotRow\([\s\S]{0,700}?tintProject\(row, projectOf\(s\)\)/.test(cliSrc),
      "renderSlots + slotRow in src/client.ts");
  }
