...
class Pane {
  ...
  constructor(readonly index: number) {
    ...
    // ⌘ combos stay with the browser (copy, paste, reload)
    this.term.attachCustomKeyEventHandler((e) => !e.metaKey);
    const updateJump = () => {
  ...
  private connect() {
    ...
    this.gen++;
    const g = this.gen;
    const ws = new WebSocket(`ws://${location.host}/ws/${this.slot}`);
    this.ws = ws;
    ws.binaryType = "arraybuffer";
    ws.onopen = () => {
      if (focused === this.index) dot.className = "on";
    ...
    ws.onclose = () => {
      ...
      if (focused === this.index) dot.className = "off";
      setTimeout(() => {
...
function focusPane(index: number) {
  ...
  ta.placeholder = slot ? `Prompt for slot ${slot === 10 ? 0 : slot}… (Enter sends)` : "Prompt… (no session in focused pane)";
...
function showSlot(id: number) {
  ...
  panes[focused].assign(id);
}

window.addEventListener("keydown", (e) => {
  if (picker.style.display === "flex" && e.key === "Escape") closePicker();
});
...
void (async () => {
  ...
  const n = view.layout && LAYOUTS[String(view.layout)] ? view.layout : 1;
